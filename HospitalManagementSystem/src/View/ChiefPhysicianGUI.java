...
public class ChiefPhysicianGUI extends JFrame {
	...
	public ChiefPhysicianGUI(final ChiefPhysician chiefphysician) throws SQLException {
		...
		btn_add.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(fld_dName.getText().length() == 0 || fld_dPass.getText().length() == 0 || fld_dID.getText().length() == 0) {
					Helper.showMsg("fill");
				}else {
					try {
						boolean control = chiefphysician.addDoctor(fld_dID.getText(), fld_dPass.getText(), fld_dName.getText());
						if(control) {
